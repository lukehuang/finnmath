...
public final class BigIntMatrix extends AbstractMatrix<BigInteger, BigIntVector, BigIntMatrix> {
    ...
    public BigIntMatrix add(final BigIntMatrix summand) {
        ...
        checkArgument(table.rowKeySet().size() == summand.rowSize(),
                "expected equal row sizes but actual %s != %s", table.rowKeySet().size(),
                summand.rowSize());
    ...
    public BigIntMatrix subtract(final BigIntMatrix subtrahend) {
        ...
        table.cellSet().forEach(cell -> {
            ...
            builder.put(rowKey, columnKey,
                    cell.getValue().subtract(subtrahend.element(rowKey, columnKey)));
        });
        return builder.build();
    }
    ...
    public BigIntMatrix multiply(final BigIntMatrix factor) {
        ...
        checkArgument(table.columnKeySet().size() == factor.rowSize(),
                "expected columnSize == factor.rowSize but actual %s != %s",
                table.columnKeySet().size(), factor.rowSize());
    ...
    public BigIntVector multiplyVector(final BigIntVector vector) {
        ...
        checkArgument(table.columnKeySet().size() == vector.size(),
                "expected columnSize == vectorSize but actual %s != %s",
        ...
        table.rowMap().forEach((rowIndex, row) -> {
            row.forEach((columnIndex, matrixEntry) -> {
                final BigInteger oldEntry = builder.element(rowIndex) != null
                        ? builder.element(rowIndex)
                        : BigInteger.ZERO;
                builder.put(rowIndex,
                        oldEntry.add(matrixEntry.multiply(vector.element(columnIndex))));
            });
        });
        return builder.build();
    }
    ...
    protected BigInteger multiplyRowWithColumn(final Map<Integer, BigInteger> row,
        ...
        checkArgument(row.size() == column.size(),
                "expected rowSize == columnSize but actual %s != %s", row.size(), column.size());
    ...
    public BigIntMatrix scalarMultiply(final BigInteger scalar) {
        ...
        final BigIntMatrixBuilder builder = builder(table.rowKeySet().size(),
    ...
    public BigInteger determinant() {
        checkState(table.rowKeySet().size() < 4, "expected rowSize < 4 but actual %s",
        ...
        return table.get(1, 1).multiply(table.get(2, 2))
                .subtract(table.get(1, 2).multiply(table.get(2, 1)));
    }

    @Override
    protected BigInteger ruleOfSarrus() {
        final BigInteger firstSummand = table.get(1, 1).multiply(table.get(2, 2))
                .multiply(table.get(3, 3));
        final BigInteger secondSummand = table.get(1, 2).multiply(table.get(2, 3))
                .multiply(table.get(3, 1));
        final BigInteger thirdSummand = table.get(1, 3).multiply(table.get(2, 1))
                .multiply(table.get(3, 2));
        final BigInteger fourthSummand = table.get(3, 1).multiply(table.get(2, 2))
                .multiply(table.get(1, 3)).negate();
        final BigInteger fifthSummand = table.get(3, 2).multiply(table.get(2, 3))
                .multiply(table.get(1, 1)).negate();
        final BigInteger sixthSummand = table.get(3, 3).multiply(table.get(2, 1))
                .multiply(table.get(1, 2)).negate();
        return firstSummand.add(secondSummand).add(thirdSummand).add(fourthSummand)
    ...
    public BigIntMatrix transpose() {
        final BigIntMatrixBuilder builder = builder(table.columnKeySet().size(),
                table.rowKeySet().size());
        table.cellSet().forEach(cell -> {
    ...
    public BigIntMatrix minor(final Integer rowIndex, final Integer columnIndex) {
        ...
        checkArgument(table.containsColumn(columnIndex),
                "expected columnIndex in [1, %s] but actual %s", table.columnKeySet().size(),
                columnIndex);
        final BigIntMatrixBuilder builder = builder(table.rowKeySet().size() - 1,
                table.columnKeySet().size() - 1);
        table.cellSet().forEach(cell -> {
            final Integer rowKey = cell.getRowKey();
            final Integer columnKey = cell.getColumnKey();
     ...
     */
    @Override
    public BigDecimal frobeniusNorm(final BigDecimal precision, final int scale,
        ...
        if (square()) {
            for (final Cell<Integer, Integer, BigInteger> cell : table.cellSet()) {
                if ((cell.getRowKey() > cell.getColumnKey())
                        && !cell.getValue().equals(BigInteger.ZERO)) {
                    return false;
                }
            }
            return true;
    ...
    public boolean invertible() {
        return determinant().equals(BigInteger.ONE.negate())
                || determinant().equals(BigInteger.ONE);
    }
     ...
     */
    @Beta
    public static final class BigIntMatrixBuilder
            extends AbstractMatrixBuilder<BigInteger, BigIntMatrix> {
         ...
         * @return {@code this}
         */
        public BigIntMatrixBuilder put(final Integer rowIndex, final Integer columnIndex,
                final BigInteger element) {
            requireNonNull(element, "element");
            requireNonNull(rowIndex, "rowIndex");
            requireNonNull(columnIndex, "columnIndex");
            checkArgument(table.rowKeySet().contains(rowIndex),
                    "expected rowIndex in [1, %s] but actual %s", table.rowKeySet().size(),
                    rowIndex);
