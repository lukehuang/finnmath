...
public final class BigIntVector extends AbstractVector<BigInteger, BigIntVector, BigDecimal> {
     ...
     * @throws IllegalArgumentException if {@code size != summand.size}
     * @author Lars Tennstedt
     * @since 1
     */
    @Override
     ...
     * @throws IllegalArgumentException if {@code size != subtrahend.size}
     * @author Lars Tennstedt
     * @since 1
     */
    @Override
     ...
     *
     * @param other The other {@link BigIntVector}
     ...
     */
    @Override
    public BigDecimal euclideanDistance(final BigIntVector other, final int scale, final RoundingMode roundingMode) {
        requireNonNull(other, "other");
        checkArgument(map.size() == other.size(), "expected equal sizes but actual %s != %s", map.size(), other.size());
        checkArgument(scale >= 0, "expected scale >= 0 but actual %s", scale);
